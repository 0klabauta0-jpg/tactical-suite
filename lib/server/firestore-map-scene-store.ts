...
export function createFirestoreMapSceneStore(): MapSceneTransactionStore {
  ...
  return {
    runObjectTransaction: (roomId, sceneId, objectId, operation) => firestore.runTransaction(async (transaction) => {
      ...
      const [objectSnapshot, boardSnapshot] = await Promise.all([transaction.get(objectRef), transaction.get(boardRef)]);
      ...
      const result = await operation({ object, groupIds: new Set(board.groups.map((group) => group.id)) });
